# --------------------------------------
# Updated first graph with simple medfly 
# growth points
# 25 Jun 2020
# NJG
# --------------------------------------
...
log_threshold(TRACE)
mylog <- tempfile()
log_appender(appender_tee(mylog))
...
lambda.vec <- unlist(t2)
names(lambda.vec) <- NULL
log_info('check this')
log_trace(cat(lambda.vec),'check here')
...
df <- data.frame(lambda=lambda.vec,temperature=temperature)
 df2 <- data.frame(lambda=emp_lam,temperature=exp_tmp)
 summary(lm(df2$lambda~poly(df2$temperature, 2,raw=TRUE)))
