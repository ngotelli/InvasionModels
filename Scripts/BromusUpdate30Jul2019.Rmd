...
I had originally intended to include one of the temperature variables, but they perform poorly compared to precip and snow cover variables, so we will start with just these initial 4. The linear regression of r versus initial density is the relationship for a simple logistic growth model. The performance of the quadratic model was virtually identical, so there is no evidence for Allee effects or other more complex functions relating per capita growth rate to density. So we will stick with the linear model. 
...
model_1 <- summary(lm(y~poly(x_1,2) + x_2 + poly(x_3,2) + poly(x_4,2)))
print(model_1)
...
lambda_model <- function(y=df_clean$r,z=best_preds){
  y <- df_clean$r
...
```{r}
qplot(x=best_preds[,1],y=df_clean$r)                                  
...
lambda_model2 <- function(y=df_clean$r,z=best_preds){
  y <- df_clean$r
  # x_1 <- z[,1] # Grow.Season.DD.all.days
  x_2 <- z[,3]  # Snow.Cover
...
```{r}
df <- data.frame(Snow.Cover=df_clean$Snow.Cover, Lambda=df_clean$r)
